# 🗓️ Sprint 1 — Sprint Planning

**Proje:** Maki Finans Koçu · **Takım:** Takım 120
**Sprint Süresi:** 19 Haziran – 5 Temmuz (2 hafta) · **Durum:** ✅ Tamamlandı

---

## 🎯 Sprint Hedefi (Sprint Goal)

> Projeyi netleştirmek; teknoloji, ürün kimliği ve gizlilik mimarisi kararlarını vermek; 3 sprintlik yol haritasını ve riskleri tanımlayarak ekibi geliştirmeye hazır hâle getirmek. *(Bu sprint tamamen planlamadır — kod yazılmaz.)*

---

## 👥 Katılımcılar

| Rol | Kişi |
|-----|------|
| Product Owner | Emir Hüseyin İnci |
| Scrum Master | Sevinç Mutlu |
| Developer | Emir Hüseyin İnci, Sevinç Mutlu |

---

## 📊 Kapasite & Taahhüt

- **Sprint kapasitesi (hedef):** ~100 SP
- **Taahhüt edilen story point:** 60 SP (US-01 → US-07)
- **Sprint uzunluğu:** 2 hafta

> Not: Sprint 1 planlama ağırlıklı olduğu için taahhüt edilen kalemler karar/dokümantasyon odaklıdır; kalan kapasite Sprint 2 hazırlığına ayrılmıştır.

---

## 📥 Sprinte Alınan Backlog Kalemleri

| ID | User Story | SP |
|----|-----------|----|
| US-01 | Problem / hedef kitle / değer önermesi netleştirme | 8 |
| US-02 | Ürün kimliği & metafor kararı (MakiKoç + orman) | 8 |
| US-03 | Teknoloji yığını seçimi | 13 |
| US-04 | Gizlilik mimarisi kararı | 13 |
| US-05 | Kategori taksonomisi taslağı | 5 |
| US-06 | 3 sprint takvimi + kapsam | 8 |
| US-07 | Risklerin belirlenmesi | 5 |
| | **Toplam** | **60** |

---

## ✅ Sprint 1 Definition of Done (DoD)

Bir kalem "Done" sayılır eğer:
- Karar yazılı olarak dokümante edildiyse,
- Product Owner tarafından onaylandıysa,
- İlgili teknik/tasarım gerekçesi netse.

**Sprint DoD:** Uygulamanın ne olduğu, hangi teknolojiyle ve hangi sırayla yapılacağı yazılı; kimlik ve gizlilik kararları verilmiş; ekip geliştirmeye başlamaya hazır.
